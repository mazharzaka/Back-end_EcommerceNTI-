...
exports.addorder = async (req, res) => {
  try {
    ...
    const Order = await order.findOneAndUpdate(
      {
        ...
        cartItem: { $elemMatch: { productId: req.body.productId } },
      },
      { $inc: { "cartItem.$.qty": 1 } },
      { new: true }
    );
    if (Order) {
      // const newQty = qty + 1;
      const CartOrders = await order
        .find({ userid })
        .populate("cartItem.productId");

      //    console.log(CartOrders);
      const total =
        CartOrders[0].cartItem.reduce(
...
exports.getcart = async (req, res) => {
  ...
  if (!CartOrders)
    return res.status(200).json({ cartItems: [], totalPriceCart: 0 });
...
exports.deleteOrderById = async (req, res) => {
  ...
  try {
    await order.findOneAndUpdate(
      ...
      {
        $pull: {
          cartItem: { productId: new mongoose.Types.ObjectId(productId) },
        },
      },
      { new: true }
    );
    ...
    console.log(
      "Existing cart items:",
      JSON.stringify(existingOrder.cartItem, null, 2)
    );
...
exports.qty = async (req, res) => {
  ...
  try {
    const updatedOrder = await order.findOneAndUpdate(
      { userid: userid, cartItem: { $elemMatch: { productId: productId } } },
      { "cartItem.$.qty": qty },
      { new: true }
    );
    const CartOrders = await order
